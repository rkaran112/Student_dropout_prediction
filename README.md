...
**Complete / working end-to-end.** The training pipeline, saved model artifacts, and all four Streamlit pages are implemented (no stub functions or TODOs found in the source). Not independently verified by running the app in this pass — review the code before deploying to production. Unit tests for the `utils/preprocess.py` helpers live in `tests/test_preprocess.py`, and for `model/train_model.py` in `tests/test_train_model.py` — covering data loading, preprocessing, and an end-to-end run of `train()` that checks the saved artifacts and metrics (run both with `pytest`); the Streamlit pages themselves are not covered by automated tests.
